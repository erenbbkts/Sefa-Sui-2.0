...
public fun list_hero(nft: Hero, price: u64, ctx: &mut TxContext) {
    ...
    // HATA DÜZELTİLDİ: emit_event -> emit (ve ctx parametresi kaldırıldı)
    event::emit(
        HeroListed {
...
public fun buy_hero(list_hero: ListHero, coin: Coin<SUI>, ctx: &mut TxContext) {
    ...
    // HATA DÜZELTİLDİ: emit_event -> emit
    event::emit(
        HeroBought {
...
public fun delist(_: &AdminCap, list_hero: ListHero) {
    ...
    // HATA DÜZELTİLDİ: 'seller: _' yerine 'seller' yazarak değişkeni aldık.
    // Çünkü aşağıda transfer ederken bu adrese ihtiyacımız var.
